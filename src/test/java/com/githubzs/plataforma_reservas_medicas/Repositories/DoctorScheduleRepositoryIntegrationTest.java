...
class DoctorScheduleRepositoryIntegrationTest extends AbstractRepositoryIT {
    ...
    void shouldFindByDoctorId() {
        ...
        // When
        var schedules = doctorScheduleRepository.findByDoctor_Id(doctor.getId());
        ...
        assertThat(schedules).extracting(ds -> ds.getDoctor().getId())
            .containsOnly(doctor.getId());
        assertThat(schedules).extracting(DoctorSchedule::getDayOfWeek)
            .containsExactlyInAnyOrder(doctorSchedule.getDayOfWeek(), doctorSchedule2.getDayOfWeek());
    }

    @Test
    @DisplayName("Doctor Schedule: No encuentra horarios para un doctor sin horarios registrados")
    void shouldReturnEmptyWhenNoSchedulesFoundForDoctor() {
        // Given
        var otherSpecialty = specialtyRepository.save(
            Specialty.builder()
                .name("Cardiología")
                .build()
        );

        var otherDoctor = doctorRepository.save(
            Doctor.builder()
                .fullName("Dr. Wilson")
        ...
        // When
        var schedules = doctorScheduleRepository.findByDoctor_Id(otherDoctor.getId());
